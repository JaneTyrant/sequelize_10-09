...
const { User } = require('../models'); // из db

module.exports.createUser = async (req, res, next) => { //public static --> create --> return Promise (он асинхр)
    try {
        const {body} = req;
...
module.exports.getAllUsers = async (req, res, next) => {
    try {
        const users = await User.findAll({ 
            attributes: { 
                exclude: ["password"],
                //exclude: ["password", "lastName"],
                //include: [["last_name", "second"]]
            },
            // where: {
            //     email: "tom3@anderson.com"
            // }
            // where: {
            //     id: { [Op.in]: [1, 2, 3] }
            // }
...
module.exports.deleteUserInstance = async (req, res, next) => {
    try {
        ...
        const result = await instanceUser.destroy();
